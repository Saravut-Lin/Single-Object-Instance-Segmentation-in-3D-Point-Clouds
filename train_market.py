...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    no_improve_epochs = 0
    patience = getattr(args, "early_stop_patience", 25)
    if args.distributed:
    ...
    # set loss func 
    # incorporate class imbalance weights if provided
    if hasattr(args, 'class_weight') and args.class_weight:
        weight = torch.tensor(args.class_weight, dtype=torch.float32).cuda()
    ...
    # Only support Market dataset
    train_transform = None
    if args.aug:
        if main_process():
            logger.info("Market augmentation")
    ...
    train_data = Market(split=args.get("train_split", "train"),
